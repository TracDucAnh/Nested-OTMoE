"""
Zero-shot XNLI evaluation for Nano-Macro-Instruct via log-likelihood scoring.

Method
------
For every (premise, hypothesis) pair we build ONE prompt and score three
candidate continuations that correspond to the three XNLI labels:

    label 0 (entailment)   -> "True"
    label 1 (neutral)      -> "Neither"
    label 2 (contradiction)-> "False"

Nano-Macro-Instruct is a chat/instruct-tuned model, so the prompt is built
with the tokenizer's chat template (`add_generation_prompt=True`) instead
of the raw "...\nAnswer:" completion-style prompt used for base models. If
the tokenizer has no chat_template (or --no_chat_template is passed), the
script falls back automatically to the original raw-prompt behavior.

For each label we score both a space-prefixed and a bare variant of its
continuation word and keep the max -- whether a leading space belongs to
the first generated token can differ across tokenizers/chat templates, and
this keeps scoring robust without hard-coding Nano-Macro-Instruct-specific
tokenization assumptions. Scores are length-normalized log-likelihoods
(teacher forcing, no sampling / no free generation), the same style used in
the GPT-3 paper and lm-evaluation-harness for ANLI/RTE/XNLI-like tasks, and
it avoids the format/parsing errors that come with free-form generation,
...
- `--no_chat_template` disables chat formatting for base-model-style runs.
"""
...
# For each label, the candidate continuation variants we'll score (leading-space
# and bare form); we keep whichever the tokenizer/model scores higher.
CANDIDATE_VARIANTS = {
