"""
Zero-shot MMMLU (Multilingual MMLU) evaluation cho baseline MidAlign (LoRA)
cua Macro-Nano-Instruct via log-likelihood scoring.

Khac voi eval/Macro-Nano-Instruct/MMMLU_eval.py (danh gia thang base model)
va eval/finetuning/Macro-Nano-Instruct/MMMLU_eval.py (danh gia baseline
Finetuning thuong), file nay:
    ...
    3. Ket qua duoc luu vao --output_dir rieng (mac dinh
       "eval/MidAlign/Macro-Nano-Instruct/results"), tach biet voi ket qua
       cua base model va cua baseline Finetuning thuong
       (eval/finetuning/Macro-Nano-Instruct/results), vi script nay nam sau
       hon 1 cap trong eval/MidAlign/.
...
Usage
-----
    python eval/MidAlign/Macro-Nano-Instruct/MMMLU_eval.py \
        ...
        --batch_size 8 \
        --output_dir eval/MidAlign/Macro-Nano-Instruct/results
...
- Resume: languages da co ket qua trong mmmlu_results.json se tu dong duoc
  bo qua, dung --overwrite_existing de danh gia lai tu dau.
"""
...
def save_results_incremental(output_dir, results, base_model, lora_adapter, lora_revision, merged, total_skipped):
...
def main():
    ...
    parser.add_argument("--lora_adapter_id", default="ducanhdinh/Macro-Nano-Instruct-MidAlign")
    ...
    parser.add_argument("--output_dir", default="eval/MidAlign/Macro-Nano-Instruct/results")
    ...
    for lang in languages:
        ...
        save_results_incremental(
            args.output_dir, results, args.base_model_name_or_path, args.lora_adapter_id,
            args.lora_revision, args.merge_adapter, total_skipped,
        )
