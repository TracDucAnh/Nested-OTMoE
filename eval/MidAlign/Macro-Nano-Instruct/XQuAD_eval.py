#!/usr/bin/env python3
"""
Zero-shot generative evaluation cua baseline MidAlign (LoRA) cua
Macro-Nano-Instruct tren XQuAD.

Khac voi eval/Macro-Nano-Instruct/XQuAD_eval.py (danh gia thang base model)
va eval/finetuning/Macro-Nano-Instruct/XQuAD_eval.py (danh gia baseline
Finetuning thuong), file nay:
    ...
    2. Gan (attach) LoRA adapter tu --lora_adapter_id, pull thang tu
       Hugging Face Hub (mac dinh "ducanhdinh/Macro-Nano-Instruct-MidAlign",
    ...
    3. Script nam sau hon 1 cap trong eval/MidAlign/ so voi ban goc
       (eval/MidAlign/Macro-Nano-Instruct/ thay vi eval/Macro-Nano-Instruct/) --
       cung do sau nhu eval/finetuning/Macro-Nano-Instruct/ nen --data_root
       mac dinh van la "../../../data/downstream/xquad" (3 cap "../" de tro
       dung ve data/downstream/xquad tu project root), khong doi so voi ban
       Finetuning.

Method (giu nguyen 100% logic cua ban goc)
------
...
Usage (run from eval/MidAlign/Macro-Nano-Instruct/):
    python XQuAD_eval.py \
        --base_model_name_or_path ATH-MaaS/Marco-Nano-Instruct \
        --lora_adapter_id ducanhdinh/Macro-Nano-Instruct-MidAlign \
...
# (dung chung quy uoc voi training/MidAlign/Macro-Nano-Instruct.py).
...
# ----------------------------------------------------------------------------- #
def main():
    parser = argparse.ArgumentParser(description="Zero-shot generative XQuAD eval cho baseline MidAlign (LoRA) Macro-Nano-Instruct")
    parser.add_argument("--base_model_name_or_path", type=str, default="ATH-MaaS/Marco-Nano-Instruct")
    parser.add_argument("--lora_adapter_id", type=str, default="ducanhdinh/Macro-Nano-Instruct-MidAlign")
    parser.add_argument("--lora_revision", type=str, default=None, help="commit/branch/tag cu the tren Hub, None = moi nhat")
    ...
    parser.add_argument("--data_root", type=str, default="../../../data/downstream/xquad",
                         help="Path to data/downstream/xquad (tinh tu eval/MidAlign/Macro-Nano-Instruct/)")
    ...
    parser.add_argument("--hf_token", type=str, default=None, help="Override HF token thu cong")
    parser.add_argument("--env_file", type=str, default=".env", help="Duong dan file .env chua HF_TOKEN")
    ...
    summary = OrderedDict()
    summary["base_model"] = args.base_model_name_or_path
    summary["lora_adapter"] = args.lora_adapter_id
    summary["lora_revision"] = args.lora_revision
    summary["merged"] = args.merge_adapter
    summary["per_language"] = OrderedDict()
    for lang, r in results.items():
        summary["per_language"][lang] = {"num_examples": r["num_examples"], "em": round(r["em"], 2), "f1": round(r["f1"], 2)}
    summary["overall"] = {
    ...
    print("-" * 46)
    for lang, r in summary["per_language"].items():
        print(f"{lang:<10}{r['num_examples']:<12}{r['em']:<10.2f}{r['f1']:<10.2f}")
    print("-" * 46)
    o = summary["overall"]
    print(f"{'OVERALL':<10}{o['num_examples']:<12}{o['em']:<10.2f}{o['f1']:<10.2f}")
    print("=" * 46)
