#!/usr/bin/env python3
"""
Zero-shot generative evaluation of Qwen1.5-MoE-A2.7B + LoRA adapter (finetuned
checkpoint) on XQuAD.

This is the LoRA-finetuned counterpart of `eval/Qwen1.5-MoE-A2.7B/XQuAD_eval.py`. The
...
normalization) is IDENTICAL to that script -- the only differences are:
  ...
  1. Model loading: we load the base model `Qwen/Qwen1.5-MoE-A2.7B` and then attach the
     ...
     training script).
  2. This file lives one level deeper (`eval/finetuning/Qwen1.5-MoE-A2.7B/` instead of
...
Usage (run from eval/finetuning/Qwen1.5-MoE-A2.7B/):
...
# ----------------------------------------------------------------------------- #
def main():
    parser = argparse.ArgumentParser(description="Zero-shot generative XQuAD eval for Qwen1.5-MoE-A2.7B + LoRA")
    ...
    parser.add_argument("--lora_model_id", type=str, default="ducanhdinh/Qwen1.5-MoE-A2.7B-Finetuning",
