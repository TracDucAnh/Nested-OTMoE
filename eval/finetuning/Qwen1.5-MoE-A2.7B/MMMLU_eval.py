"""
Zero-shot MMMLU (Multilingual MMLU) evaluation for Qwen1.5-MoE-A2.7B + LoRA adapter
(finetuned checkpoint) via log-likelihood scoring.

This is the LoRA-finetuned counterpart of `eval/Qwen1.5-MoE-A2.7B/MMMLU_eval.py`. The
...
def score_chunk_with_oom_retry(model, tokenizer, chunk, build_prompt_fn, device, min_batch_size=1):
    ...
    # See eval/Qwen1.5-MoE-A2.7B/MMMLU_eval.py for the full explanation: we deliberately
    # do NOT call clear_memory() or recurse from *inside* the `except` block, since the
    # live exception/traceback would otherwise keep the failed batch's GPU tensors
    # pinned in memory for the whole duration of that block.
    oom = False
    try:
...
def _atomic_write_bytes(path: str, data: bytes) -> None:
    ...
    fd, tmp_path = tempfile.mkstemp(prefix=".tmp_mmmlu_lora_", dir=directory)
    try:
...
def write_all_outputs(output_dir, base_model_name, lora_model_id, lora_revision, merged,
    ...
    summary = {
        ...
        "lora_revision": lora_revision or "main",
        "merged": merged,
...
def main():
    ...
    for lang in to_run:
        ...
        _, _, _, _, checkpoint_path = write_all_outputs(
            args.output_dir, args.model_name_or_path, args.lora_model_id, args.lora_revision,
            args.merge_lora, results, subject_totals, total_skipped
        )
