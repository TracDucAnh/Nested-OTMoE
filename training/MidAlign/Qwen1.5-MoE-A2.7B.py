...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--find_unused_parameters", dest="find_unused_parameters",
                    ...
                    help="[FIX NCCL Watchdog SIGABRT] BAT BUOC = True (mac dinh) khi finetune "
                         ...
                         "so num_experts (vd 4/60), nen o mot step/GPU bat ky rat de co (cac) "
                         "expert LoRA khong nhan duoc token nao -> khong co gradient. Neu DDP "
                         "dat find_unused_parameters=False, Reducer se CHO VO HAN gradient cua "
                         "cac expert do trong backward() -> sau --nccl_timeout_minutes phut, "
                         "NCCL Watchdog coi la deadlock va bien no thanh SIGABRT (exit code -6), "
                         "dung ca cac rank khac. Chi tat bang --no_find_unused_parameters neu "
                         "chac chan MOI expert trong layer duoc LoRA deu duoc it nhat 1 token "
                         "dung tren MOI GPU o MOI step (vd batch rat lon / khong dung MoE).")
